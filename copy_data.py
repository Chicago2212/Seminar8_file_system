...
def copy_row():
    ...
    birthdate = data[3]
    town = data[4]
    with open(f'Seminar8_file_system/db/data_{nf_2}.txt', 'r', encoding='utf-8') as file:
        data_2 = file.readlines()
    now_number_row = len(data_2) + 1
    with open(f'Seminar8_file_system/db/data_{nf_2}.txt', 'a', encoding='utf-8') as file:
